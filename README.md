...
- 支持拖拽缩略图调整图片顺序，并同步保存到 `image_list`
- 图片列表自适应节点可用空间，节点缩放时自动调整布局
- 上传、选择文件夹、粘贴图片均默认追加，不覆盖现有列表
- 工具栏精简为：上传、选择文件夹、粘贴、单张入队、逐张入队、清空
- 点击缩略图即可选中单张图片并同步 `index`，排序或删除后索引自动跟随
- 缩略图使用懒加载和异步解码，减少大量图片时的内存和重绘开销
